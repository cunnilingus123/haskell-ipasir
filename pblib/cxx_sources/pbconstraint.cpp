...
void PBConstraint::print(bool hugo) const
{
  ...
  for (int i = 0; i < getN(); ++i)
  {
	...
	if (hugo) cerr << weighted_literals[i].weight << " -x" << -weighted_literals[i].lit << " +";
	...
	if (hugo) cerr << weighted_literals[getN() - 1].weight << " -x" << -weighted_literals[getN() - 1].lit;
  ...
  if (comparator == LEQ)
    if(hugo) cerr << " =< " << leq << endl;
    else cout << " =< " << leq << endl;
  else if (comparator == GEQ)
    if(hugo) cerr << " >= " << geq << endl;
    else cout << " >= " << geq << endl;
  else
    if(hugo) cerr << " >= " << geq << " =< " << leq << endl;
    else cout << " >= " << geq << " =< " << leq << endl;
  
}

void PBConstraint::printGeq(bool hugo) const
{
  if (comparator != LEQ)
  {
    print();
...
void PBConstraint::printNoNL(bool hugo) const
{
  if (getN() == 0)
  {
    if (hugo)

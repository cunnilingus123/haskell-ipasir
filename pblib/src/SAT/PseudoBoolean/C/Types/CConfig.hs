...
class CardinalityMethod a where
    ...
    just_approximate = fromBool . isJust . approximate
...
instance CardinalityMethod AtMostOne where
    ...
    use_recursive_bdd_test Config { config = Just a@AmoBdd{} } = fromBool $ amoRecursive a
    use_recursive_bdd_test _ = false
    
    use_real_robdds Config { config = Just a@AmoBdd{} } = fromBool $ amoRealdRoBdds a
    use_real_robdds _ = false
...
instance CardinalityMethod AtMostK where
    ...
    use_recursive_bdd_test Config { config = Just a@AmkBdd{} } = fromBool $ amkRecursive a
    use_recursive_bdd_test _ = false
    
    use_real_robdds Config { config = Just a@AmkBdd{} } = fromBool $ amkRealdRoBdds a
    use_real_robdds _ = false
...
instance CardinalityMethod PseudoBoolean where
    ...
    use_gac_binary_merge Config { config = Just a@PbBinaryMerge{} } = fromBool $ useGac a
    use_gac_binary_merge _ = false

    use_watch_dog_encoding_in_binary_merger Config { config = Just a@PbBinaryMerge{} } = fromBool $ noSupportForSingeBits a
    use_watch_dog_encoding_in_binary_merger _ = false
